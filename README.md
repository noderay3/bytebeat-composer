...
- **OS media-key wiring** — macOS Now Playing, iOS lock screen, and Android notification controls all route to play/pause/next/previous via `navigator.mediaSession`. **Caveat:** if another audio app (Apple Music, Spotify, VLC…) is actively playing or has the media-key focus, macOS routes hardware keys to it instead. If the play/pause / next / prev keys don't reach this page, close or pause the other app first.
