...
- **OS media-key wiring** — macOS Now Playing, iOS lock screen, and Android notification controls all route to play/pause/next/previous via `navigator.mediaSession`. A silent looping `<audio>` element keeps the session alive; its pause behavior is platform-specific (desktop pauses it with bytebeat so F7–F9 toggle correctly, mobile keeps it looping through pause so the lock-screen widget and Bluetooth earbuds' play/pause survive a pause). **Caveat:** on desktop, if another audio app (Apple Music, Spotify, VLC…) is actively playing or has the media-key focus, macOS routes hardware keys to it instead. If the play/pause / next / prev keys don't reach this page, close or pause the other app first.
